c LOCALISED VERSION OF getmom.f and disort.f modified relative to the
c original DISORT code.
c
c MODIFICATIONS ARE:
c 07/02/13, G Thomas: Check on UTAU (done in CHEKIN, Line: 4983) changed
c                     from an absolute difference to TAUC(NLYR) of 1e-4,
c                     to a relative difference of 1e-4
c 08/02/13, G Thomas: Altered error handling and changed many sub-
c                     routines, including GETMOM and DISORT themselves,
c                     to functions that return error flags. A return
c                     value of 0 or .FALSE. means no error. Now "fatal"
c                     errors are detected by the code will not result in
c                     an abort signal, but will GETMOM or DISORT will
c                     return "1" as their value. This is so that these
c                     errors will not result in IDL aborting when
c                     calling the DISORT DLM.
...
c     ..
     ...
     E  9*0.002, 18*0.001 /
      ...
      IF ( IPHAS.LT.1 .OR. IPHAS.GT.5 ) THEN
         CALL ERRMSG( 'GETMOM--bad input variable IPHAS',.TRUE.)
         GETMOM = 1
         RETURN
      ENDIF

      IF ( IPHAS.EQ.3 .AND. (GG.LE.-1.0 .OR. GG.GE.1.0) ) THEN
...
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      INTEGER FUNCTION DISORT(NLYR,DTAUC, SSALB, NMOM, PMOM, TEMPER,
...
c     .. Parameters ..
     ...
     &          MXSQT
      PARAMETER ( MXCLY = 32, MXULV = 11, MXCMU = 64, MXUMU = 40,
     &          MXPHI = 11, MI = MXCMU / 2, MI9M2 = 9*MI - 2,
...
c     .. Local Scalars ..
      ...
      LOGICAL   COMPAR, CORINT, DELTAM, LYRCUT, PASS1, ERROR
...
c     .. External Functions ..
      ...
      INTEGER   CHEKIN
      LOGICAL   ALBTRN, CMPINT, FLUXES, INTCOR, LEPOLY, PRAVIN,
     &          PRTINP, PRTINT, SETDIS, SETMTX, SLFTST, SOLEIG, SOLVE0,
     &          SURFAC, TERPEV, TERPSO, UPBEAM, UPISOT, USRINT, ZEROAL
      EXTERNAL  PLKAVG, R1MACH, RATIO, CHEKIN
     &          ALBTRN, CMPINT, FLUXES, INTCOR, LEPOLY, PRAVIN,
     &          PRTINP, PRTINT, SETDIS, SETMTX, SLFTST, SOLEIG, SOLVE0,
     &          SURFAC, TERPEV, TERPSO, UPBEAM, UPISOT, USRINT, ZEROAL
c     ..
c     .. External Subroutines
...
c     On detecting an error the code will arrive here, where
c     RFLDIR will be set to an error code and DISORT will return
 181  IF( ERROR ) THEN
         DISORT = 1
         RETURN
      ENDIF
...
c                            ** Be sure SLFTST sets all print flags off.
         ...
         ERROR = SLFTST( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM,
     &        DTAUC( 1 ),
...
c                                ** Check input dimensions and variables
      ...
      INERROR = CHEKIN( NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO,
...
c                                 ** Zero internal and output arrays
      ...
      ERROR = ZEROAL( MXCLY, EXPBEA(1), FLYR, OPRIM, PHASA, PHAST,
     &     PHASM, TAUCPR(1), XR0, XR1,
...
c                                 ** Perform various setup operations
      ...
      ERROR = SETDIS( CMU, CWT, DELTAM, DTAUC, DTAUCP, EXPBEA, FBEAM,
     &     FLYR, GL, IBCND, LAYRU, LYRCUT, MAXMOM, MAXUMU, MXCMU,
...
c                                 ** Print input information
      IF( PRNT( 1 ) ) THEN
         ERROR = PRTINP( NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM,
     &        TEMPER, WVNMLO, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
...
c                              ** beam angles at once
      IF( IBCND.EQ.1 ) THEN
         ...
         ERROR = ALBTRN( ALBEDO, AMB, APB, ARRAY, B, BDR, CBAND, CC,
     &        CMU, CWT, DTAUCP, EVAL, EVECC, GL, GC, GU, IPVT, KK,
...
c                                   ** Calculate Planck functions
      ...
      ELSE
         ...
         IF( ( TPLANK.LT.0.0 ).OR.( BPLANK.EQ.-999.0) ) GOTO 181

         DO 40 LEV = 0, NLYR
            PKAG( LEV ) = PLKAVG( WVNMLO, WVNMHI, TEMPER( LEV ) )
            IF( PKAG( LEV ).EQ.-999.0 ) GOTO 181
   40    CONTINUE
...
c                             **     cosines
         IF( FBEAM.GT.0.0 ) THEN
            ...
            ERROR = LEPOLY( NCOS, MAZIM, MXCMU, NSTR-1, ANGCOS, SQT,
     &                      YLM0 )
            IF ( ERROR ) GOTO 181
         ...
         IF( .NOT.ONLYFL .AND. USRANG ) THEN
            ERROR = LEPOLY( NUMU, MAZIM, MXCMU, NSTR-1, UMU, SQT, YLMU )
            IF ( ERROR ) GOTO 181
         ENDIF

         ERROR = LEPOLY( NN, MAZIM, MXCMU, NSTR-1, CMU, SQT, YLMC )
         IF ( ERROR ) GOTO 181
...
c                                 ** and emissivity properties
         IF( .NOT.LYRCUT ) THEN
            ERROR = SURFAC( ALBEDO, DELM0, CMU, FBEAM, LAMBER, MI,
...
c                      ** STWL(23f); return eigenvalues and eigenvectors
            ...
            ERROR = SOLEIG( AMB, APB, ARRAY, CMU, CWT, GL( 0,LC ), MI,
...
c                                  ** beam source
            IF( FBEAM.GT.0.0 ) THEN
               ERROR = UPBEAM( ARRAY, CC, CMU, DELM0, FBEAM, GL( 0,LC ),
...
c
     ...
     &             ( PKAG( LC ) - PKAG( LC-1 ) ) / DTAUCP( LC )
               ...
               ERROR = UPISOT( ARRAY, CC, CMU, IPVT, MXCMU, NN, NSTR,
...
c                                            ** to user angles
               ...
               ERROR = TERPEV( CWT, EVECC, GL( 0,LC ), GU( 1,1,LC ),
     &              MAZIM, MXCMU, MXUMU, NN, NSTR, NUMU, WK, YLMC,
     &              YLMU )
               IF ( ERROR ) GOTO 181
...
c                                            ** to user angles
               ...
               ERROR = TERPSO( CWT, DELM0, FBEAM, GL( 0,LC ), MAZIM,
     &              MXCMU, PLANK, NUMU, NSTR, OPRIM( LC ), PI, YLM0,
...
c                      ** boundary and layer interface conditions
         ...
         ERROR = SETMTX( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC, KK,
...
c                      ** geneous solution (general boundary conditions)
         ...
         ERROR = SOLVE0( B, BDR, BEM, BPLANK, CBAND, CMU, CWT, EXPBEA,
...
c                                  ** Compute upward and downward fluxes
         ...
         IF( MAZIM.EQ.0 ) THEN
            ERROR = FLUXES( CMU, CWT, FBEAM, GC, KK, LAYRU, LL, LYRCUT,
...
c                                     ** components at user angles
            ...
            ERROR = USRINT( BPLANK, CMU, CWT, DELM0, DTAUCP, EMU,
     &           EXPBEA, FBEAM, FISOT, GC, GU, KK, LAMBER, LAYRU, LL,
...
c                                     ** components at quadrature angles
            ...
            ERROR = CMPINT( FBEAM, GC, KK, LAYRU, LL, LYRCUT, MAZIM,
     &           MXCMU, MXULV, MXUMU, NCUT, NN, NSTR, PLANK, NTAU,
...
c                              ** at user angles
            ...
            IF( PRNTU0( 2 ) ) THEN
               ERROR = PRAVIN( UMU, NUMU, MXUMU, UTAU, NTAU, U0U )
               IF ( ERROR ) GOTO 181
            ENDIF
...
c                                    ** corrections
      ...
      IF( CORINT ) THEN
         ERROR = INTCOR( DITHER, FBEAM, FLYR, LAYRU, LYRCUT, MAXMOM,
...
c                                          ** Print intensities
      ...
      IF( PRNT( 3 ) .AND. .NOT.ONLYFL ) THEN
         ERROR = PRTINT( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI, MAXULV,
     &                   MAXUMU )
         IF ( ERROR ) GOTO 181
      ENDIF
...
c                                    ** correct answers and abort if bad
         ...
         ERROR = SLFTST( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM,
     &        DTAUC( 1 ), FBEAM, FISOT, IBCND, LAMBER, NLYR, PLANK,
     &        NPHI, NUMU, NSTR, NTAU, ONLYFL, PHI( 1 ), PHI0, NMOM,
     &        PMOM( 0,1 ), PRNT, PRNTU0, SSALB( 1 ), TEMIS,
     &        TEMPER( 0 ), TTEMP, UMU( 1 ), USRANG, USRTAU,
     ...
     &        FLUP( 1 ), RFLDIR( 1 ), RFLDN( 1 ), UU( 1,1,1 ) )
      ...
      LOGICAL FUNCTION ASYMTX( AA, EVEC, EVAL, M, IA, IEVEC, IER, WKD,
     &                         AAD, EVECD, EVALD )
...
c                         ** Put results into output arrays
  ...
  720 CONTINUE
      ...
      LOGICAL FUNCTION CMPINT( FBEAM, GC, KK, LAYRU, LL, LYRCUT, MAZIM,
     &     MXCMU, MXULV, MXUMU, NCUT, NN, NSTR, PLANK, NTAU,
     &     TAUCPR, UMU0, UTAUPR, ZZ, ZPLK0, ZPLK1, UUM )
...
c                                       ** Loop over user levels
   ...
   40 CONTINUE
      ...
      LOGICAL FUNCTION FLUXES( CMU, CWT, FBEAM, GC, KK, LAYRU, LL,
     &     LYRCUT, MAXULV, MXCMU, MXULV, NCUT, NN, NSTR, NTAU,
...
c                                                ** this level
  ...
  100    CONTINUE
      ...
      LOGICAL FUNCTION INTCOR( DITHER, FBEAM, FLYR, LAYRU, LYRCUT,
     &     MAXMOM, MAXULV, MAXUMU, NMOM, NCUT, NPHI, NSTR, NTAU,
     &     NUMU, OPRIM, PHASA, PHAST, PHASM, PHIRAD, PI,
     &     RPD, PMOM, SSALB, DTAUC, TAUC, TAUCPR, UMU,
     &     UMU0, UTAU, UTAUPR, UU )
...
c                              ** Eq. STWL (A.13)
     ...
     &         PSPIKE * XIFUNC( -UMU, UMU0P, UMU0P, UTAU )
      ...
      LOGICAL FUNCTION SETDIS( CMU, CWT, DELTAM, DTAUC, DTAUCP, EXPBEA,
     &     FBEAM, FLYR, GL, IBCND, LAYRU, LYRCUT, MAXMOM, MAXUMU,
...
c
     ...
     &   .NOT.DELTAM )  CORINT = .FALSE.
      ...
      LOGICAL FUNCTION SETMTX( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC,
     &     KK, LAMBER, LYRCUT, MI, MI9M2, MXCMU, NCOL, NCUT,
     &     NNLYRI, NN, NSTR, TAUCPR, WK )
...
c                                 ** Downward intensity, Eq. STWL (65d)
   ...
   30    CONTINUE
      ...
      LOGICAL FUNCTION SOLEIG( AMB, APB, ARRAY, CMU, CWT, GL, MI, MAZIM,
...
c                      ** Find (real) eigenvalues and eigenvectors
      ...
      SOLEIG = ASYMTX( ARRAY, EVECC, EVAL, NN, MI, MXCMU, IER, WKD, AAD,
     &                 EVECCD, EVALD )
      IF( SOLEIG ) RETURN

      IF( IER.GT.0 ) THEN

c         WRITE( *, '(//,A,I4,A)' ) ' ASYMTX--eigenvalue no. ',
c     &      IER, '  didnt converge.  Lower-numbered eigenvalues wrong.'
...
c                                ** reversing sign of 'k' in SS(10) )
  ...
  130 CONTINUE
      ...
      LOGICAL FUNCTION SOLVE0( B, BDR, BEM, BPLANK, CBAND, CMU, CWT,
     &     EXPBEA, FBEAM, FISOT, IPVT, LAMBER, LL, LYRCUT, MAZIM,
...
c                   ** necessary to prevent errors
  ...
  190 CONTINUE
      ...
      LOGICAL FUNCTION SURFAC( ALBEDO, DELM0, CMU, FBEAM, LAMBER, MI,
     &     MAZIM, MXUMU, NN, NUMU, ONLYFL, PI, UMU, UMU0,
     &     USRANG, WVNMLO, WVNMHI, BDR, EMU, BEM, RMU )
...
c                               ** user angle cosines -UMU-.
  ...
  170    CONTINUE
      ...
      LOGICAL FUNCTION TERPEV( CWT, EVECC, GL, GU, MAZIM, MXCMU, MXUMU,
...
c                                    ** and store eigenvectors
   ...
   50 CONTINUE
      ...
      LOGICAL FUNCTION TERPSO( CWT, DELM0, FBEAM, GL, MAZIM, MXCMU,
     &     PLANK, NUMU, NSTR, OPRIM, PI, YLM0, YLMC, YLMU, PSI0,
...
c
   ...
   80    CONTINUE
      ...
      LOGICAL FUNCTION UPBEAM( ARRAY, CC, CMU, DELM0, FBEAM, GL, IPVT,
     &     MAZIM, MXCMU, NN, NSTR, PI, UMU0, WK, YLM0, YLMC, ZJ, ZZ )
...
c                ** ZJ;  return solution(s) in ZJ
   ...
   40 CONTINUE
      ...
      LOGICAL FUNCTION UPISOT( ARRAY, CC, CMU, IPVT, MXCMU, NN, NSTR,
     &     OPRIM, WK, XR0, XR1, Z0, Z1, ZPLK0, ZPLK1 )
...
c                       ** except ZJ replaced by Z1 and Z0
   ...
   40 CONTINUE
      ...
      LOGICAL FUNCTION USRINT( BPLANK, CMU, CWT, DELM0, DTAUCP, EMU,
     &     EXPBEA, FBEAM, FISOT, GC, GU, KK, LAMBER, LAYRU, LL,
     &     LYRCUT, MAZIM, MXCMU, MXULV, MXUMU, NCUT, NLYR,
     &     NN, NSTR, PLANK, NUMU, NTAU, PI, RMU, TAUCPR,
     &     TPLANK, UMU, UMU0, UTAUPR, WK, ZBEAM, Z0U, Z1U,
...
c ******************************************************************
      ...
      INTEGER FUNCTION CHEKIN( NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER,
     &                   WVNMLO, WVNMHI, USRTAU, NTAU, UTAU, NSTR,
     &                   USRANG, NUMU, UMU, NPHI, PHI, IBCND, FBEAM,
     &                   UMU0, PHI0, FISOT, LAMBER, ALBEDO, BTEMP,
     &                   TTEMP, TEMIS, PLANK, ONLYFL, DELTAM, CORINT,
     &                   ACCUR, TAUC, MAXCLY, MAXULV, MAXUMU, MAXPHI,
     &                   MAXMOM, MXCLY, MXULV, MXUMU, MXCMU, MXPHI,
     &                   MXSQT )

c           Checks the input dimensions and variables
c   CHANGED BY G THOMAS 08/02/2013: AS A RESULT OF ERRMSG NO LONGER
c   CAUSING AN ABORT SIGNAL, THIS SUBROUTINE HAS ALSO BEEN ALTERED. IF
c   A FATAL ERROR IS DETECTED, THE ROUTINE RETURNS A "1", WHICH THEN
c   CAUSES DISORT TO IMMEDIATELY RETURN TO THE CALLING ROUTINE.
c   NB. THIS MEANS CHEKIN HAS BEEN CONVERTED FROM A SUBROUTINE TO A
C       INTEGER VALUED FUNCTION
...
c     ..
      ...
      CHEKIN = 1 ! Assume and error is present and change if not found
      INPERR = .FALSE.
   ...
   30 CONTINUE
      ...
      ELSE IF( USRTAU ) THEN
         ...
         DO 40 LU = 1, NTAU
c           CHANGED BY GARETH THOMAS (05/02/2013)
c           IF( ABS( UTAU( LU )-TAUC( NLYR ) ).LE.1.E-4 )
...
c                       ** Loop over angle of reflection
   ...
   30 CONTINUE
     ...
     &    CALL ERRMSG( 'DREF--albedo value not in (0,1)',.False. )
      ...
      LOGICAL FUNCTION LEPOLY( NMU, M, MAXMU, TWONM1, MU, SQT, YLM )
...
C     I REMOVED ABOVE LINE AND REPLACED IT WITH A FUNCTION CALL
C     IN LINES ABOVE THIS FUNCTION  S MARSH JAN 2003
c     ..
...
c                                    ** WNUMLO and WNUMHI both large
     ...
     &    .FALSE.)
      ...
      LOGICAL FUNCTION PRAVIN( UMU, NUMU, MXUMU, UTAU, NTAU, U0U )
...
c      WRITE( *, '(//,A)' )
c     &   ' *******  AZIMUTHALLY AVERAGED INTENSITIES ' //
c     &   '(at user polar angles)  ********'

      LENFMT = 8
      NPASS  = 1 + (NUMU-1) / LENFMT

c      WRITE( *,'(/,A,/,A)') '   Optical   Polar Angle Cosines',
c     &                      '     Depth'
...
c         WRITE( *,'(/,10X,8F14.5)') ( UMU(IU), IU = IUMIN, IUMAX )

         DO 10 LU = 1, NTAU
c            WRITE( *, '(0P,F10.4,1P,8E14.4)' ) UTAU( LU ),
c     &           ( U0U( IU,LU ), IU = IUMIN, IUMAX )
   10    CONTINUE
   ...
   20 CONTINUE
      ...
      LOGICAL FUNCTION PRTINP( NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM,
     &     TEMPER, WVNMLO, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
     &     NPHI, PHI, IBCND, FBEAM, UMU0, PHI0, FISOT,
     &     LAMBER, ALBEDO, BTEMP, TTEMP, TEMIS, DELTAM,
     &     PLANK, ONLYFL, CORINT, ACCUR, FLYR, LYRCUT,
...
c                                       ** two WRITEs above)
   ...
   20    CONTINUE
      ...
      LOGICAL FUNCTION PRTINT( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI,
     &     MAXULV, MAXUMU )
...
c      WRITE( *, '(/,A,/,A,/,A)' )
c     &   '             Polar   Azimuth angles (degrees)',
c     &   '   Optical   Angle',
c     &   '    Depth   Cosine'
...
c            WRITE( *, '(/,18X,10F11.2)' ) ( PHI(J), J = JMIN, JMAX )

c            IF( NP.EQ.1 ) WRITE( *, '(F10.4,F8.4,1P,10E11.3)' )
c     &             UTAU(LU), UMU(1), (UU(1, LU, J), J = JMIN, JMAX)
c            IF( NP.GT.1 ) WRITE( *, '(10X,F8.4,1P,10E11.3)' )
c     &                       UMU(1), (UU(1, LU, J), J = JMIN, JMAX)

            DO 10 IU = 2, NUMU
c               WRITE( *, '(10X,F8.4,1P,10E11.3)' )
c     &                 UMU( IU ), ( UU( IU, LU, J ), J = JMIN, JMAX )
   10       CONTINUE
   ...
   30 CONTINUE
      ...
      LOGICAL FUNCTION QGAUSN( M, GMU, GWT )
...
c     ..
     ...
     &          TWO / 2.D0 /
      ...
      IF( M.LT.1 ) THEN
         CALL ERRMSG( 'QGAUSN--Bad value of M',.True.)
         QGAUSN = .TRUE.
         RETURN
      ENDIF
...
c                      ** from A*B because A*B may (over/under)flow
     ...
     &       ( A.LT.0.0 .AND. B.GT.0.0 ) ) RATIO = -RATIO
      ...
      LOGICAL FUNCTION SLFTST( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM,
     &     DTAUC, FBEAM, FISOT, IBCND, LAMBER, NLYR, PLANK, NPHI,
...
c                                      ** Restore user input values
   ...
   80    CONTINUE
      ...
      LOGICAL FUNCTION ZEROAL( ND1, EXPBEA, FLYR, OPRIM, PHASA, PHAST,
     &                        PHASM, TAUCPR, XR0, XR1,
...
c ******************************************************************
      ...
      LOGICAL FUNCTION ALBTRN( ALBEDO, AMB, APB, ARRAY, B, BDR, CBAND,
     &                   CC, CMU, CWT, DTAUCP, EVAL, EVECC, GL, GC, GU,
...
c     ..
c     .. Functions
      LOGICAL LEPOLY, SETMTX, SOLEIG, TERPEV
c     ..
c     .. External Subroutines ..

      EXTERNAL  ALTRIN, ERRMSG, PRALTR, SGBCO, SOLEIG, SOLVE1,
     &          SPALTR, ZEROIT
c     ..
c     .. Intrinsic Functions ..
...
c                          ** and user polar angle cosines
      ...
      ALBTRN = LEPOLY( NUMU, MAZIM, MXCMU, NSTR-1, UMU, SQT, YLMU )
      IF( ALBTRN ) RETURN

      ALBTRN = LEPOLY( NN, MAZIM, MXCMU, NSTR-1, CMU, SQT, YLMC )
      IF( ALBTRN ) RETURN
...
c                                       ** in Eq. STWJ(8b), STWL(23f)
         ...
         ALBTRN = SOLEIG( AMB, APB, ARRAY, CMU, CWT, GL( 0,LC ), MI,
     &        MAZIM, MXCMU, NN, NSTR, YLMC, CC, EVECC, EVAL,
     &        KK( 1,LC ), GC( 1,1,LC ), AAD, EVECCD, EVALD,
     &        WKD )
         IF( ALBTRN ) RETURN
...
c                          ** Interpolate eigenvectors to user angles
         ...
         ALBTRN = TERPEV( CWT, EVECC, GL( 0,LC ), GU( 1,1,LC ), MAZIM,
...
c                      ** LINPACK routines)
      ...
      ALBTRN = SETMTX( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC, KK,
     &     LAMBER, LYRCUT, MI, MI9M2, MXCMU, NCOL, NCUT,
     &     NNLYRI, NN, NSTR, TAUCPR, WK )
      IF( ALBTRN ) RETURN
