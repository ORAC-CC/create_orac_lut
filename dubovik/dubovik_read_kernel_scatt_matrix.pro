pro dubovik_read_kernel_scatt_matrix, file, dd, grid1=grid1
...
;       gridpath = strjoin(gridpath[0:n_elements(gridpath)-2])
   ...
   Nn =  long(Nn)
   Nk = -long(Nk)
   ; Now start on the values of K11
   KK = dblarr(NN, NK, NR, NT)
   RI_n = fltarr(NN)
   RI_k = fltarr(NK)
   ...
   name = 'K'+string(element, format='(i02)')
